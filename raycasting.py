...
def load_textures():
    ...
    try:
        ...
        print("Failed to load a texture:", e)


def render_sky(win):
    """Renders the sky with a deep blue color."""
    sky_color = (48, 25, 52)
    pygame.draw.rect(win, sky_color, (0, 0, SCREEN_WIDTH, SCREEN_HEIGHT // 2))

def cast_rays(win, player):
    """Renders walls and gates."""
...
def cast_floor(win, player):
    """Renders the floor with textures."""
...
def check_gate_interaction(player):
    """Checks if the player is near the gate and interacts."""
    player_row = int(player.y / TILE_SIZE)
    player_col = int(player.x / TILE_SIZE)
    gate_row, gate_col = 11, 30  # Gate position

    if abs(player_row - gate_row) <= 1 and abs(player_col - gate_col) <= 1:  # Check proximity
        return True
    return False
